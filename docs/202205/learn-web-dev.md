...
第一次遇到这个问题，我试着回忆了一下自己是如何学习 js 的。似乎并没有特别快速的学习方法。不过，前端开发又或说 web 应用开发是一种可以有快速反馈的学习，而且就是一个慢慢深入的过程。

前端技术栈中有三种技术最为基础：js、html、css。这三种技术也分别对应一个应用的三层：交互、布局、样式。

## html、css

html 使用了 xml 的语法来表示一个网页。web 1.0 时代，html 仅提供了基本的富文本展示功能。html 通过嵌套标签来表示一个网页的布局。css 负责控制各个标签的展示效果，也就是样式。

html 现在已经发展到 html5，而 css 也到 css3.0。html、css 的标准基本都是浏览器实现的，各种浏览器对于这些标准支持程度不一样。因些不同的浏览器之间会存在了兼容性问题。对于是 ie 浏览器与现代浏览器之间兼容性问题。

我所谓的现代浏览器是一种 blink/webkit/gecko 内核为基础的 ever green 浏览器。

### html

对于 html 基本上认为是 xml 就可以了。
...
html 标准中包含许多标签。在编写布局时具体使用哪个标签其实没有绝对的标签。对于入门，只需要记住这几个：
...
css 可以实现很多效果。但是对于前端开发而言掌握一部分基础的样式就算上手了。css 语法带有强烈的 c 风格，属性使用 key-value 写法。比如：
...
盒子模型会影响一个标签的展示。具体的细节可以问问 google。现在流行的盒子模型是 borderbox。

## JS

js，更正式地应该叫 JavaScript、ECMAScript。js 有 web 技术栈中主要是负责交互部分，包括人机交互、机机交互。

js 由 tc39 组织负责进行标准化和版本迭代，目前是一年一个版本。最新版本是 ES2022。关于 js 的历史可以找找其他资料看看，绝对是充满传奇的故事。

### 语法

js 也属于 c 语法的一支。大概是就是 c 语法 + scheme 功能 + oo 的神奇组合。
...
总结 js 的语法：语法并不复杂可细节不少，有 c 风格语言背景的同学应该很容易上手。
...
比如在浏览器中，可以使用 Docment、fetch、console 等 API，这些都是由浏览器提供。

现在的 js 除浏览器外，还有 node.js、deno 这类面向后台服务的执行环境。它们都会特供不同的宿主 API 和工具。

### 现代 js 开发流程

现在的前端应用，特别是 SPA 应用变得越来越复杂。js 的开发体验也变得越来越工程化。特别是 node.js 出现后，用 js 进行软件开发的体验有了明显的进步。
...
vuejs 是目前流行的前端三大框架之一（另两个是 angular、react）。据我所知，vue 在小厂中非常流行，原因是上手真的简单，而且原创也是华人。

无论 vue、angular，还是 react 都是基于 mvvm 或类似概念开发的框架。追求的是通过组件化的带来更好的开发体验。

所谓组件化是将一个大的功能模块细分为多个小组件。采用类似于积木的方式组合更大更复杂的应用。

我认为组件也可以分为多种类型：

- 基础组件，比如按钮、菜单等一般由组件库提供，而且比较通用。甚至跨端通过
- 通用业务组件，比如各个系统菜单。这种组件在特定项目中有一定的通用性
- 具体业务组件，这些都是按照业务流程编写组件，几乎没有通过性
...
> vue2、vue3 大部分场景下是兼容的。不过不代表代码可以直接复用！新开项目的话，推荐 vue3。
